...
**専用のターミナルを 1 つ開いて**実行します（プロジェクト作業用のターミナルとは別に開いてください）。
...
起動するとコンソールに以下のように `SECRET_TOKEN` が表示されます。

```
✅ Approval server running on http://localhost:3000

🔑 SECRET_TOKEN: xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
   (スマホUIとエージェントコードにこのトークンを設定してください)
```

このトークンは後でスマートフォンの UI に入力します。`APPROVAL_TOKEN` 環境変数を設定しない場合は起動ごとに異なるランダムな値が生成されます。

**複数のプロジェクトで Claude Code を同時に使う場合も、サーバーは 1 つだけ起動すれば OK です。**

### 2. ngrok でトンネルを開く

**別のターミナルを新たに開いて**実行します。

```bash
ngrok http 3000
```

起動すると以下のような URL が表示されます。

```
Forwarding  https://xxxx.ngrok-free.app -> http://localhost:3000
```

この `https://xxxx.ngrok-free.app` の URL をメモしておきます（スマホ UI に入力します）。
...
セットアップ（フック設定・インストール）は初回のみです。2 回目以降は以下の順で操作します。
...
1. **ターミナル A** でサーバーを起動する
   ...
   ```bash
   node /path/to/claude-approval-server/approval-server.js
   ```

2. **ターミナル B** で ngrok を起動する

   ```bash
   ngrok http 3000
   ```

3. スマートフォンのブラウザで ngrok URL を開き、URL と `SECRET_TOKEN` を入力して接続する
4. **ターミナル C 以降**でプロジェクトを開き、Claude Code を起動する

### 停止順序

1. 各プロジェクトの Claude Code を終了する（`Ctrl+C` または `/exit`）
2. ngrok を停止する（`Ctrl+C`）← **先に止める**（外部からのアクセスを閉じる）
3. サーバーを停止する（`Ctrl+C`）

> **注意**: ngrok を先に止めると外部からサーバーへアクセスできなくなります。逆の順序（サーバー → ngrok）でも動作上の問題はありませんが、ngrok を先に止めるほうがセキュリティ上望ましいです。
...
On startup, the `SECRET_TOKEN` is printed to the console:

```
✅ Approval server running on http://localhost:3000

🔑 SECRET_TOKEN: xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
   (Set this token in the smartphone UI and agent code)
```

You will enter this token in the smartphone UI later. If `APPROVAL_TOKEN` is not set, a new random token is generated each time the server starts.
...
Note the `https://xxxx.ngrok-free.app` URL — you will enter it in the smartphone UI.
...
The setup steps (hook configuration, installation) are one-time only. From the second time onward:
...
2. **Terminal B** — Start ngrok:

   ```bash
   ngrok http 3000
   ```

3. Open the ngrok URL on your smartphone, enter the URL and `SECRET_TOKEN` to connect.
4. **Terminal C and beyond** — Open your projects and start Claude Code.

### Shutdown Order

1. Exit Claude Code in each project (`Ctrl+C` or `/exit`)
2. Stop ngrok (`Ctrl+C`) ← **stop this first** (closes external access)
3. Stop the server (`Ctrl+C`)

> **Note**: Stopping ngrok first prevents external access to the server. Stopping in the reverse order (server → ngrok) causes no functional issue, but stopping ngrok first is preferable for security.
