...
**v1.21.0 以降、巡回後に確認画面から先頭タブへ戻る Shift+Tab(借り返し)にも、上記の背景色セルの属性検証が必須になりました。** 従来は借り(自分が右へ押した分)があれば属性検証より先に許可していましたが、「属性を確認できないフレームでは Shift+Tab を送らない」を例外なしの不変条件にするための変更です(属性検証を通過しても「CLI が描いた行」の証明にはならない点は上記のとおり変わりません)。代償として、確認画面でバー行の背景色が読めない過渡フレームに当たると戻れなくなり、転送を諦めて次項の notice でスマホへ PC 操作を誘導します。録画 corpus の再生では確認画面フレームはすべて属性が読め、通常運用でのこの可用性低下は観測されていません。

### 複合質問の転送を諦めたとき、スマホに「PC で操作してください」という notice が出る

タブ式複合質問の巡回後、確認画面から先頭タブへ戻れなかったとき(= 上で述べた理由で転送を諦めるとき)、スマホの承認パネルに「PC で操作してください」という情報カード(`kind:'notice'`)が表示されます。

- **notice は承認ではありません**: 選択肢・自由記入・タブを持たず、「確認しました」ボタンで消えるだけです。PTY への注入経路には接続しません(wrapper は notice の結果を poll しません)
- **自動で消える経路は 3 つ**: PC 側でタブ UI が見えなくなったとき / wrapper 側の 30 分 TTL / サーバー側の backstop(pending から 60 分経過後に次回 GC 走査で削除。GC は 5 分間隔なので、実際に消えるまで通常最大で約 65 分かかります)
...
**Since v1.21.0, the Shift+Tab that returns to the first tab after a sweep (paying back the "debt") also requires the background-color attribute check described above.** It used to be allowed ahead of the attribute check whenever a debt was outstanding; now "never send Shift+Tab on a frame whose attributes cannot be confirmed" is an exceptionless invariant (passing the attribute check still does not prove the row was drawn by the CLI, as noted above). The trade-off: if the confirmation screen's tab-bar row lands on a transient frame whose background color cannot be read, the wrapper can no longer rewind, gives up forwarding, and falls back to the notice card described next to direct you to the PC. Replays of the recorded corpus show every confirmation-screen frame with readable attributes, so this availability cost has not been observed in normal operation.

### A notice card telling you to use the PC appears instead of a forwarded dialog

When the wrapper cannot rewind back to the first tab after sweeping a tabbed multi-question dialog (i.e. when it gives up forwarding for the reason above), the approval panel on the phone shows an information card (`kind:'notice'`) asking you to answer on the PC.

- **A notice is not an approval**: it has no options, no free text, and no tabs — tapping "Acknowledged" just dismisses it. It never connects to the PTY injection path (the wrapper never polls a notice for a result).
