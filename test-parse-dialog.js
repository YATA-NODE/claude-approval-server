...
// -------------------------------------------------------
// 13b. parseDialog: 全角「？」(U+FF1F) で終わる日本語プロンプトを検出する
//   回帰: qIdx = lastIndexOf('?') (ASCII のみ) だと全角「？」プロンプトで
//   qIdx=-1 → null となり、日本語 AUQ がスマホへ一切転送されなかった。
// -------------------------------------------------------
console.log('\n[13b] parseDialog: 全角「？」プロンプト検出')
{
  const lines = [
    '╭──────────────────────────────────────────────╮',
    '│ ←  ☐ 進め方  ☐ ヘルプ原本  ✔ Submit  →        │',
  ...
  const ascii = full.replace('？', '?') // 半角「?」(従来から通っていた形)

  const rFull = parseDialog(full)
  assertEq('全角「？」でも parseDialog 検出 → ok', !!rFull, true)
  if (rFull) {
    assertEq('全角: tool = AskUserQuestion', rFull.tool, 'AskUserQuestion')
    assertEq('全角: prompt 本文が抽出される', rFull.prompt.includes('どの順序'), true)
    assertEq('全角: prompt 末尾が ？', rFull.prompt.endsWith('？'), true)
    assertEq('全角: option 5 個', rFull.options.length, 5)
  }
  assertEq('全角: isTabbedDialog も true', isTabbedDialog(full), true)

  // 半角でも従来どおり同じ形状で取れる(全角追加が半角を壊していない)
  const rAscii = parseDialog(ascii)
  assertEq('半角「?」でも parseDialog 検出 → ok', !!rAscii, true)
  if (rFull && rAscii) {
    assertEq('全角と半角で option 数が一致', rFull.options.length, rAscii.options.length)
  }
}
